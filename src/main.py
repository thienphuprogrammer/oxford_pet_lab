...
def _build_model(task: str, backbone: str, cfg: Config) -> tf.keras.Model:
...
def _prepare_datasets(
    ...
    train_ds = augmentor.create_augmentation_dataset(
        train_ds,
    ...
    # Save stats
    with open(cfg.PROJECT_ROOT / "train_stats.json", "w") as f:
        json.dump(stats_train, f)
    with open(cfg.PROJECT_ROOT / "val_stats.json", "w") as f:
        json.dump(stats_val, f)
    with open(cfg.PROJECT_ROOT / "test_stats.json", "w") as f:
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Model & trainer
    # ------------------------------------------------------------------
    model = _build_model(args.task, args.backbone, cfg)
    trainer = Trainer(
        model=model,
        task_type=args.task,
